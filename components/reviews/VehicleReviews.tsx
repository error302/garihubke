...
export function VehicleReviews({ vehicleId }: VehicleReviewsProps) {
  ...
  useEffect(() => {
    fetch(`/api/reviews?vehicleId=${vehicleId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold">Vehicle Reviews</h3>
        <WriteReviewButton targetType="vehicle" targetId={vehicleId} onSuccess={() => window.location.reload()} />
      </div>
